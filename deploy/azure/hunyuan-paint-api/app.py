...
def _run_paint_pipeline(mesh_path: Path, image_path: Optional[Path], output_path: Path):
    ...
    try:
        return model(mesh_path=str(mesh_path), output_path=str(output_path), **kwargs)
    except TypeError:
        try:
            return model(str(mesh_path), output_path=str(output_path), **kwargs)
        except TypeError:
...
async def texturize(request: PaintRequest, authorization: str | None = Header(default=None)):
    ...
    mesh_suffix = ".obj" if request.output_format == "obj" else ".glb"
    ...
    output_path = TEMP_ROOT / f"paint_{uuid.uuid4().hex[:10]}.{request.output_format}"

    try:
        result = _run_paint_pipeline(mesh_path, image_path, output_path)
        final_output = _resolve_output_path(result, output_path)
        return FileResponse(
            str(final_output),
